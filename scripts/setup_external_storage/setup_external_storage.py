...
def read_args():
    ...
    parser.add_argument(
            "--synapse-parent",
            help="Synapse ID of the parent folder/project")
    parser.add_argument(
            "--synapse-folder-name",
            help=("Name of the Synapse folder to use with the external "
                  "storage location."))
    parser.add_argument(
            "--s3-bucket",
            help="S3 bucket name")
    ...
    parser.add_argument(
            "--configure-bucket",
            action="store_true",
            help=("!! Overwrites any existing bucket policy !!"
...
def configure_bucket(
    ...
    with open("owner.txt", "rb") as f:
        s3_client.put_object(
                Body=f,
                Bucket=bucket_name,
                Key=os.path.join(bucket_key or "", "owner.txt"))
    # Write CORS
