...
def read_args():
    parser = argparse.ArgumentParser(
        description=(
            "Construct a SQL query string for use with "
    ...
    parser.add_argument(
        "--dataset-mapping", help="The path to the dataset mapping file."
    )
    parser.add_argument("--dataset", help="The name of the dataset.")
    parser.add_argument(
        "--dataset-version", help="The version of the dataset, formatted 'v*'."
    )
    parser.add_argument(
        "--file-view",
        help=("The Synapse ID of a file view containing " "files to be sampled."),
    )
...
def sample_app_versions(syn, relevant_app_versions, file_view):
    ...
    query_str = (f"SELECT * FROM {file_view} " "WHERE appVersion in ") + app_version_str
    app_version_q = syn.tableQuery(query_str)
    app_version_df = app_version_q.asDataFrame()
    app_version_df_sample = app_version_df.sample(len(app_version_df)).drop_duplicates(
        subset="appVersion"
    )
...
def main():
    ...
    sampled_record_ids = sample_app_versions(
        syn=syn, relevant_app_versions=relevant_app_versions, file_view=args.file_view
    )
    record_id_str = "('" + "','".join(sampled_record_ids) + "')"
    query_str = ("SELECT * FROM {source_table} " "WHERE recordId in ") + record_id_str
    print(query_str)
    return query_str
