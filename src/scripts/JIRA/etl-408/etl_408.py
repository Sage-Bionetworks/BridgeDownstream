...
def get_invalid_records(aws_session, queue_url, iterations, delete_after_sample=False):
    ...
    for i in range(iterations):
        try:
            ...
            print("Encountered the following error while polling the SQS queue. "
...
def main():
    ...
    reference = get_missing_records(
            syn=syn,
            study=STUDY
    )
    parquet_base_s3_uri = f"s3://{PARQUET_BUCKET}/bridge-downstream/{APP}/{STUDY}/parquet"
