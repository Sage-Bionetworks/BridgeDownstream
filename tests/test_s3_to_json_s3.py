...
class TestS3ToJsonS3:
    ...
    @pytest.fixture
    def metadata_json_self_ref(self):
        metadata_json = {
            "schemaRevision": 1,
            "dataGroups": "",
            "versionString": "1",
            "activityLabel": "Word Problems",
            "endDate": "2023-05-09T10:36:27.754-06:00",
            "startDate": "2023-05-09T10:35:50.650-06:00",
            "schemaIdentifier": "MTBVerbalReasoning",
            "taskRunUUID": "SAMPLEUUIS",
            "scheduledActivityGuid": "SAMPLEGUID",
            "appName": "MobileToolboxApp",
            "deviceInfo": "SAMPLE_DEVICE_INFO",
            "scheduledOn": "00:00:00",
            "deviceTypeIdentifier": "SAMPLE_DEVICE_ID",
            "taskIdentifier": "VerbalReasoningV1",
            "scheduleIdentifier": "SOME_ID",
            "rsdFrameworkVersion": "???",
            "appVersion": "version 1.5.0, build 108",
            "files": [
                {
                    "jsonSchema": "https://raw.githubusercontent.com/MobileToolbox/MobileToolboxSchemas/0.9.0/VerbalReasoning/VerbalReasoningJSONSchema.json",
                    "filename": "taskData.json",
                    "identifier": "VerbalReasoningV1",
                    "contentType": "application/json",
                    "timestamp": "2023-05-09T10:36:27.051-06:00",
                }
            ],
        }
        yield metadata_json
    ...
    def test_that_get_self_ref_schema_list_returns_empty_if_not_self_ref(self, s3_obj):
        with zipfile.ZipFile(io.BytesIO(s3_obj["Body"])) as z:
            schema_list = s3_to_json_s3.get_self_ref_schema_list(zipfile_obj=z)
            assert schema_list == {}

    def test_that_get_self_ref_schema_list_returns_nonempty_dict_if_self_ref(
        self, s3_obj, metadata_json_self_ref
    ):
        with patch.object(
            zipfile.ZipFile,
            "namelist",
            return_value=["metadata.json", "info.json", "taskData.json"],
        ) as patch_namelist, patch.object(
            json, "load", return_value=metadata_json_self_ref
        ) as patch_json_load, patch.object(
            zipfile.ZipFile, "open"
        ):
            with zipfile.ZipFile(io.BytesIO(s3_obj["Body"])) as z:
                schema_list = s3_to_json_s3.get_self_ref_schema_list(zipfile_obj=z)
                assert schema_list == {
                    "taskData.json": "https://raw.githubusercontent.com/MobileToolbox/MobileToolboxSchemas/0.9.0/VerbalReasoning/VerbalReasoningJSONSchema.json"
                }

    def test_that_get_self_ref_schema_list_returns_empty_if_no_metadata_json(
        self, s3_obj
    ):
        with patch.object(
            zipfile.ZipFile, "namelist", return_value=["info.json", "taskData.json"]
        ) as patch_namelist:
            with zipfile.ZipFile(io.BytesIO(s3_obj["Body"])) as z:
                schema_list = s3_to_json_s3.get_self_ref_schema_list(zipfile_obj=z)
                assert schema_list == {}

    def test_that_get_json_schema_returns_expected_settings_for_self_ref(self):
        with patch.object(
            s3_to_json_s3, "_get_cached_json_schema", return_value="some_schema"
        ) as patch_cache_json_schema:
            self_ref_schema_list = {
                "taskData.json": "https://raw.githubusercontent.com/MobileToolbox/MobileToolboxSchemas/0.9.0/VerbalReasoning/VerbalReasoningJSONSchema.json"
            }
            file_metadata = {
            ...
            json_schema = s3_to_json_s3.get_json_schema(
                archive_map=None,
                file_metadata=file_metadata,
                json_schemas={},
            ...
            assert json_schema == {
                "url": self_ref_schema_list["taskData.json"],
                "schema": "some_schema",
